            ...
            for t_line in t_invoice_line:
                if t_line[2] and 'product_id' in t_line[2]:
                    ...
                    if t_product_data:
                        ...
                        if (t_type != 'service' and t_product_data.net_mass is None):
                            raise orm.except_orm(_('Error!'),
                                                 _('Net mass not defined for the product!'))
                        if (t_type != 'service' and t_product_data.statistical_value is None):
                            raise orm.except_orm(_('Error!'),
                                                 _('Statistical Value not defined for the product!'))
                        if (t_type != 'service' and not t_product_data.uom_secondary):
                            raise orm.except_orm(_('Error!'),
                                                 _('Secondary Unit of Measure not defined for the product!'))

        res = super(account_invoice_intracee, self).create(cr, uid, vals, context)
        return res
