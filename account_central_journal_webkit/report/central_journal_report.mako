...
<body>
    <%
        ...
        result_rows = get_move_lines(date_from, date_to)
    ...
    %for line in result_rows :
        ...
        % if new_page == True:
            ...
            <tr class="p_row p_row_page">
                ...
                <td colspan="2" class="p_cell p_cell_page"><span class="p_text p_page">${ _("Page:") }&nbsp;${print_info['year_name']}/${progr_page}</span></td>
            </tr>
            
            <tr class="p_row p_row_head">
                <td class="p_cell p_cell_head"><span class="p_text">${ _("Row") }</span></td>
                <td class="p_cell p_cell_head"><span class="p_text">${ _("Date Reg.") }</span></td>
                <td class="p_cell p_cell_head"><span class="p_text">${ _("Date Doc.") }</span></td>
                <td class="p_cell p_cell_head"><span class="p_text">${ _("N° Doc.") }</span></td>
        ...
        <tr class="p_row">
            <td class="p_cell ${style} p_cell_progr_row"><span class="p_text p_progr_row">${progr_row}</span></td>
            <td class="p_cell ${style} p_cell_date"><span class="p_text p_date">${ formatLang(line.get('mdate'), date=True) or ''|entity }</span></td>
            <td class="p_cell ${style} p_cell_date"><span class="p_text p_date">${ formatLang(line.get('mdocument_date'), date=True) or ''|entity }</span></td>
            <td class="p_cell ${style} p_cell_ref"><span class="p_text p_ref">${ line.get('mdocument_number') or ''|entity }</span></td>
            <td class="p_cell ${style} p_cell_ref"><span class="p_text p_ref">${ line.get('lref') or ''|entity }</span></td>
            <td class="p_cell ${style} p_cell_move_id_name"><span class="p_text p_move_id_name">${ line.get('mname') or ''|entity }</span></td>
            <td class="p_cell ${style} p_cell_account_id_code"><span class="p_text p_account_id_code">${ line.get('account_code') or ''|entity }</span></td>
            <td class="p_cell ${style} p_cell_account_id_name"><span class="p_text p_account_id_name">${ line.get('account_name') or ''|entity }</span></td>
            <td class="p_cell ${style} p_cell_name"><span class="p_text p_name">${ line.get('lname') or ''|entity }</span></td>
            <td class="p_cell ${style} p_cell_debit"><span class="p_text p_debit">${ formatLang(line.get('ldebit'), digits=get_digits(dp='Account')) |entity }</span></td>
            <td class="p_cell ${style} p_cell_credit"><span class="p_text p_credit">${ formatLang(line.get('lcredit'), digits=get_digits(dp='Account')) |entity }</span></td>
        </tr>
        <%
        debit_tot = debit_tot + line.get('ldebit')
        credit_tot = credit_tot + line.get('lcredit')
        %>        
