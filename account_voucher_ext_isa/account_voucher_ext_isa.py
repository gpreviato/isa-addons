...
class account_voucher_ext_isa(osv.osv):
...
#        return res
    ...
    #_name = 'account.voucher.ext.isa'
    _name = 'account.voucher'
    _inherit = 'account.voucher'
    _description = 'Accounting Voucher Ext ISA'
...
#        return res
    ...
    def onchange_amount(self, cr, uid, ids, amount, rate, partner_id, journal_id, currency_id, ttype, date, payment_rate_currency_id, company_id, context=None):
        if context is None:
            context = {}
        elif context.has_key('type2'):
            if context['type2']== 'payment_without_cn': 
                self._without_cn = False
            else:
                self._without_cn = True
        else:
            self._without_cn = True
        ...
        if self._without_cn:
            ...
            res = self.recompute_voucher_lines_without_cn(cr, uid, ids, partner_id, journal_id, amount, currency_id, ttype, date, context=context)    
        #import os.sys
        #print(str(res))
        #res['value']['line_cr_ids']
        
#        lenght_cr_ids=len(res['value']['line_cr_ids'])
#        lenght_dr_ids=len(res['value']['line_dr_ids'])
#        for i in range(lenght_cr_ids):
#            res['value']['line_cr_ids'][i]['reconcile']=False
#            res['value']['line_cr_ids'][i]['amount']= 0.0
#            res['value']['line_cr_ids'][i]['amount_unreconciled']=res['value']['line_cr_ids'][i]['amount_original']
#            
#        for j in range(lenght_dr_ids):    
#            res['value']['line_dr_ids'][j]['reconcile']=False
#            res['value']['line_dr_ids'][i]['amount']= 0.0
#            res['value']['line_dr_ids'][i]['amount_unreconciled']=res['value']['line_cr_ids'][i]['amount_original']
#            
...
#        return res
    ...
    def recompute_voucher_lines_without_cn(self, cr, uid, ids, partner_id, journal_id, price, currency_id, ttype, date, context=None):
        """
        Returns a dict that contains new values and context
        ...
        if not context.get('move_line_ids', False):
            ...
            aj_ids = move_line_pool.read(cr, uid, ids, ['journal_id'])
            
            #ids = move_line_pool.search(cr, uid, [('state','=','valid'), ('account_id.type', '=', account_type), ('reconcile_id', '=', False), ('partner_id', '=', partner_id), ('')], context=context)
            
            #account_ids = move_line_pool.read(cr, uid, ids, ['account_id'])
            
            #account_pool = self.pool.get('account_account')
            #ids_account = account_pool.search(cr, uid, account_ids, context=context)
            journal_pool_for_dn = self.pool.get('account.journal')
            #aj_ids_dict_list = journal_pool_for_dn.read(cr, uid, ids, ['default_credit_account_id'])
            #aj_ids = []
            #for i in range(len(aj_ids_dict_list)):
            #    a_id = aj_ids_dict_list[i]['account_id'][0]
                #aj_ids_dict_list[i]['default_credit_account_id']
            #    aj_ids.append(a_id)
            aml_ids = ids    
            ids_without_dn_all = []
            for i in aj_ids:
                #ids_account = account_pool.search(cr, uid, [('id','=', i), ('code', '!=', 'ECNJ')], context=context)
                ci=i['journal_id'][0]
                aj_ids_without_dn = journal_pool_for_dn.search(cr, uid, [('id','=', ci), ('code', '!=', 'ECNJ')], context=context)
                for cj in aj_ids_without_dn:
                      am_ids_without_nd = []
                      for ck in aml_ids:
                          am_ids_without_nd = move_line_pool.search(cr, uid, [('id','=',ck), ('journal_id','=',cj)], context=context)
                          if am_ids_without_nd:
                              am_id_without_nd = am_ids_without_nd[0]
                              if (not (am_id_without_nd in ids_without_dn_all)):
                                    ids_without_dn_all.append(am_id_without_nd)
                          #ids_without_dn_all.extend(am_ids_without_nd)
                          
                      
                    #am_ids_without_nd = move_line_pool.search(cr, uid, [('id','=',cj)], context=context)
                    #ids_without_dn_all.extend(am_ids_without_nd)
                    #ids.extend(ids_without_dn)
                    #ids = ids_without_dn 
        else:
            ids = context['move_line_ids']
        
        #fa scomparire tutte le righe di credito e di debito, sostanzialmente la maschera de voucher,
        # cioè del pagamento, appare senza ne righe di fatture ne righe di note di credito 
        #ids = []
        ids = ids_without_dn_all
...
#class account_voucher_line_ext_isa(osv.osv):
