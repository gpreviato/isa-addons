...
# TODO gestire date uguali
#    _sql_constraints = [
#        ('document_date_check', "CHECK ( (document_date <= date))", "The Document Date must be before the Registration Date !")
#    ]
                ...
                if not move.protocol_number and move.journal_id.iva_registry_id:
                    ...
                    if not invoice_ids:
                        ...
                        for invoice_data in invoice_obj.browse(cr, uid, invoice_ids, context=context):
                            if invoice_data.protocol_number:
                                self.write(cr, uid, [move.id],
                                           {'protocol_number': invoice_data.protocol_number})
