...
class riba_unsolved(orm.TransientModel):
    ...
    _columns = {
        ...
        'payment_type': fields.selection([('C', 'Contante'),
                                          ('B', 'Bonifico'),
                                          ('D', 'Ricevuta Bancaria')],
                                         'Tipo Pagamento',),
        }
    ...
    def create_move(self, cr, uid, ids, context=None):
        ...
        if not wizard.unsolved_journal_id or not wizard.bank_account_id or not wizard.bank_expense_account_id:
        ...
        for move_line in move_pool.browse(cr, uid, move_id, context=context).line_id:
            if move_line.account_id.id == distinta_line.partner_id.property_account_receivable.id:
                for riba_move_line in distinta_line.move_line_ids:
                    ...
                    move_line_pool.write(cr, uid, move_line.id, {
                                'unsolved_move_originator_id': riba_move_line.move_line_id.invoice.move_id.id,
                                }, context=context)
